...
include_graphics('../results/13_confirmed_progress.png')
```
...
```{r out.width=c('50%', '50%'), fig.show='hold'}
include_graphics(c('../results/13_confirmed_age.png', '../results/12_age_deceased_ratio.png'))
```
...
cluster_pred <- read_tsv("../results/wrangled_cluster_pred.tsv") %>% 
...
ann_pred <- read_tsv("../results/wrangled_ann_pred") %>%
  mutate(accuracy = str_c(round(acc*100, 1), "%")) %>%
  select(accuracy)
